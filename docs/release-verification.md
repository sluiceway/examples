...
| 22 | A change that moved before apply | Pu | Twice. After the tick: the bot ticks `greeting:prod`, the test bed's `apply` job waits at its first step while the branch `release-verify-hold-apply` exists, the driver pushes a change to that stack and waits for its scan, then deletes the branch. A stale row: the scan job waits while `release-verify-hold-scan` exists, the driver pushes a change to `greeting:dev` and ticks its row as the scan before wrote it, then releases the scan. | Each time, as records 0008, 0051 and 0052 and the checklist's item 9 say: one record, ended `error`, "the change moved since the tick"; `apply` red with `outcome` `refused`, its result file's reason and the log line `<stack> was not deployed: the change moved since the tick.`; one comment that mentions the ticker, word for word; the row pending with a box, a new hash, `failed="true"` and the line `:x: last deploy failed: the change moved since the tick · ticked by <bot>`. After the tick, as record 0111 adds: `apply`'s log says `main moved on from <commit>, the commit this run checked out, to a commit that changes a file <stack> claims: <file>.` and that it started a full scan, it runs no fresh preview, the scan it dispatched writes the row, and the comment says a newer commit reached the branch before the deploy started. | Yes | 5 min |
| 23 | A cancelled deploy, then "Re-run failed jobs" | Pu | A push of `pulumi/states/slow`, whose deploy sleeps for five minutes. The bot ticks it, and ten seconds after its record is `in_progress` the driver cancels the run. Then it calls "Re-run failed jobs" on that run. | After the cancel: the record ends `error`, "the run ended without a result"; `settle` logs that it ended the record, `Wrote the failure line on the row of <stack>, before the full scan previews it again (record 0113).` and that it started a full scan; the edit history holds `settle`'s row, `no preview since its deploy ended, the next scan previews it` with state `preview-failed`, `failed="true"`, no box and the failure line right under it, within 60 seconds of the cancel; the scan after it leaves `:x: last deploy failed: the run ended without a result · ticked by <bot>` and `failed="true"` on the row. After the re-run: no new record and no new status; attempt 2 of `apply` is red with `outcome` `refused` and logs `Deployment record N already ended as error. Nothing is deployed. A re-run never deploys (record 0019).` and `This deploy already ended. Tick the box on the dashboard to try again.`; `settle` logs that no record of the run is open; the row is as before. | Yes | 8 min |
...
| 30 | A hostile name stays plain text | Tofu | A resource of `opentofu/notes` named `#1 @sluiceway www.example.com *x*` through `for_each`. `@sluiceway` is the organisation that owns the test bed: GitHub links a mention only when the account exists, so a handle nobody holds would never show whether the name was escaped, and a mention of an organisation notifies no person. | The name is on the row as record 0112 writes it: `<span>#</span>1 <span>@</span>sluiceway www&#46;example.com &#42;x&#42;`. In the issue body as GitHub renders it (`application/vnd.github.html+json`), and in the stack's preview page rendered through `POST /markdown` in the test bed's context, the name holds no issue link, no mention, no autolink and no emphasis. | No | 0, part of 1 |
...
| 35 | A tick inside a deploy freeze | Pu | A push adds `freezes` with one freeze that started a minute ago and ends about eight minutes later, with a reason. The driver ticks `greeting:dev` inside it, waits for its end, then starts the workflow with "Run workflow". | The dashboard names the freeze once: `Deploy freeze until <date> <time> UTC (<reason>): every deploy waits for it to end.` The tick opens one record with `window: true` that stays open, starts no `apply`, and the row is `queued`, with no box, and reads `queued for the end of the deploy freeze (<reason>) at <date> <time> UTC · ticked by <login>`. After the end the dispatched `resolve` ends it `inactive`, "started in a later run", and opens a record with the same hash and ticker and no `window`, which `apply` deploys; the row is in sync and the freeze line is gone (record 0115). | Yes | 12 min |
| 36 | Layout keys keep the destroy lines | Pu, Tofu | A push sets `dashboard.pendingDetail: compact` and `dashboard.inSyncSection: off`, and takes the freeze of 35 out. A second push takes both keys out. | Against the body just before: every row is still there and its marker is the same but for `shortened`; a pending row shows only lines the full row had, no fold of changes and no attribution line, and keeps every `:warning: DELETE`, `:warning: REPLACE` and failure line; the in sync rows are whole inside `<details><summary>N stacks in sections this dashboard does not show</summary>`, with no In sync heading and no fold of ignored stacks, and the counts line still counts them; the destroy alert stays. After the second push the fold is gone, and the heading, the ignored stacks and the folds of changes are back (record 0114). | No | 4 min |
| 37 | Counts on a pending row's marker | Pu, Tofu | Nothing, reads every step. | For every pending row the last scan of a step previewed: `creates`, `updates`, `replaces` and `tracking` on the marker are the numbers of the result file's changes and of the first line, each left out at 0, and `destroys` and `deletes` agree with them. Across the run at least one row has a create, an update, a replace and a delete (record 0110). `tracking` and `behind` are not met: no fixture moves a resource, and no scenario queues a stack behind another. | No | 0 |
