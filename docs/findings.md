...
| 4 | A tick in the browser starts a run; `waiting to start`, then `deploying`, `ticked by` | 1 | Split | 6, at `v0.26.1` | Scenario part passed: the tick starts `resolve`, and the edit history holds a body with `waiting to start · ticked by <ticker>` and a later one with `deploying · ticked by <ticker>`. Person: one tick with a mouse, since the driver ticks through the API. |
| 5 | Deploy succeeds; in sync, under Recently deployed, record task `sluiceway:<stack id>` | 1 | Covered | 6, at `v0.26.1` | The record ends `success` with task `sluiceway:<stack id>` (`src/core/deployment.ts:47`), the row is in sync, and the first line under Recently deployed names the stack, the ticker and the run. |
...
| 10 | Cancel during `apply`; within a minute the row says the run ended without a result | 1 | Automated | 23, fails at `v0.26.1` on the minute | Passed at `v0.26.1`: the record ends `error`, "the run ended without a result" (`src/core/failure-reason.ts:95`), `settle` logs that it ended it and started a full scan, and that scan puts the failure line and `failed="true"` on the row. The line came 101 seconds after the cancel, not within a minute: see [the finding](#2026-09-23-a-cancelled-deploy-shows-on-its-row-after-the-next-scan-not-within-a-minute). |
| 11 | "Re-run failed jobs"; nothing deploys, the summary says to tick again | 1 | Split | 23, at `v0.26.1` | Scenario part passed: after "Re-run failed jobs" no new record or status, attempt 2 of `apply` red with `outcome` `refused` and the log lines `Deployment record N already ended as error. Nothing is deployed. A re-run never deploys (record 0019).` and `This deploy already ended. Tick the box on the dashboard to try again.` (`src/modes/apply.ts:242-244`), `settle` finds no open record, the row as before. Person: the summary itself, which no API reads. |
...
| 13 | Close the dashboard; the next scan reopens the same number | 1 | Covered | 25, at `v0.26.1` | The same number open again with its label and its pin, no second issue, the log line `Reopened the dashboard and wrote it:` (`src/modes/scan.ts:1204-1221`). |
| 14 | `dashboard.redact: true`; no type, name or property name in the issue | 1 | Covered | 26, at `v0.26.1` | No change line, and none of the types, names or changed paths the result file lists is in the body; each pending row sends to the summary; the result file still holds the changes (`docs/notifications.md:115`). |
| 15 | `dashboard.personality: false`; no image, the dry line | 1 | Automated | 26, new assertion | Passed at `v0.26.1`: no `<picture>`, no mascot URL, no dot on the counts line or the trail. Still to assert: the dry line of `src/render/voice.ts:39-43`, which shows only when every stack is in sync or there is none, a state the order does not reach yet. |
...
| 41 | A change to the shared package gives a full scan | 3 | Covered | 4, at `v0.26.1` | A push of a file that no stack claims and `scan.unrelated` does not list previews every stack, with the log line `fell back to a full scan: no stack claims <file>` and the file in the group `Changed files that no stack claims` (`src/modes/scan.ts:816-835`). |
| 42 | A full scan stays far below 1,000 requests | 3 | Split | 1, at `v0.26.1` | Scenario part passed: `The scan made N requests to the GitHub API.` (`src/modes/scan.ts:274`), N below 1,000. Person: the private repo's number. |
...
| 45 | Note the state of the chosen stack and two others | 4 | Split | 6, at `v0.26.1` | Scenario part passed: the saved state before and after the second tick of 6; the ticked stack's state changed, the five other stacks' state files are byte for byte the same. Person: the real backend's serials. |
...
| 47 | One `apply`; `resolve` and `settle` on a hosted runner, no tool call | 4 | Split | 6, at `v0.26.1` | Scenario part passed: `resolve`'s matrix holds one stack, one `apply` job, and the logs of `resolve` and `settle` hold no tool run while `apply`'s does. Person: the private repo's runners and token. See below: the one-job workflow cannot do this part. |
| 48 | The row says `deploying`, `ticked by`, box gone | 4 | Covered | 6, at `v0.26.1` | The edit history holds a body where the row reads `deploying · ticked by <ticker>` with no box. |
| 49 | The deploy succeeds and the change is live | 4 | Needs a person | | Setup: a real cluster, checked by eye. |
| 50 | In sync; first under Recently deployed; the record carries the commit | 4 | Covered | 6, at `v0.26.1` | The first line under Recently deployed names the stack, the ticker and the run; the record's `sha` is the commit the tick deployed. |
| 51 | The two other stacks were not touched | 4 | Split | 6, at `v0.26.1` | Scenario part passed: no record for any other stack, every other row with its state and hash as before, and every other stack's state file byte for byte the same. Person: the real backend. |
| 52 | The next full scan changes nothing on the chosen row | 4 | Covered | 6, at `v0.26.1` | After a dispatched full scan the row block is byte for byte the one the deploy left. |
...
| 54 | A second change is pending again, and names its pull request | 4 | Covered | 29, at `v0.26.1` | A pull request that changes a stack deployed from the dashboard, merged with a squash: the row is pending and its attribution line starts `from #N by <author>` and ends with a compare link (`src/core/attribution.ts:212-216`). |
