...
|---|---|---|
| Covered | 13 | A scenario proved the whole item against the tag the row names. |
| Automated | 8 | A planned or new scenario can prove the whole item. No person needed. |
...
| 51 | The two other stacks were not touched | 4 | Split | 6, at `v0.26.1`, new assertion | Scenario part passed: no record for any other stack, and every other row with its state and hash as before. Still to assert: their state files. Person: the real backend. |
