...
| [`driver/scans.ts`](driver/scans.ts), [`ticks.ts`](driver/ticks.ts), [`signs.ts`](driver/signs.ts), [`results.ts`](driver/results.ts), [`names.ts`](driver/names.ts), [`settings.ts`](driver/settings.ts), [`moved.ts`](driver/moved.ts), [`size.ts`](driver/size.ts), [`cancel.ts`](driver/cancel.ts), [`attribution.ts`](driver/attribution.ts), [`pair.ts`](driver/pair.ts), [`merge.ts`](driver/merge.ts), [`layout.ts`](driver/layout.ts), [`state.ts`](driver/state.ts) | The assertions: scenarios 1 to 5, 6 and 7, 17 and 18, 19, 30, 24 to 26 and 28, 22, 27, 23, 29 and 21, 31 to 34, 35 to 37, and the saved state of each deploy. |
...
The test bed is shared: the driver first waits until the newest commit of its `main` ends in `: done`, and whatever happens it leaves a commit that does when it ends, unless `--keep` holds the test bed. It keeps the logs of every run of the test bed the moment the run ends, in `logs/` of its output. Without `--version` it verifies `SLUICEWAY_VERSION` of `versions.json`; `--version v0.26.0` verifies another tag. It takes about 70 minutes, twenty of them waits: for the deploy window of scenario 33 to open and for the deploy freeze of scenario 35 to end. The summary is printed at the end and written, with everything the driver read, to `release-verify/out/<version>-<time>/`, which git ignores. `--keep` leaves the test bed as the last step left it, to look at by hand; the next run's reset clears it. The exit code is 0 when every scenario passed.
